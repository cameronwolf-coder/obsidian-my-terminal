...
function stripAnsiRaw(text) {
  return text.replace(/\x1b\[[0-9;]*[ -/]*[@-~]/g, "").replace(/\x1b\][\s\S]*?(?:\x07|\x1b\\)/g, "").replace(/\x1b[^[]/g, "").replace(/\u00a0/g, " ").replace(/\r\n/g, "\n");
}
...
var WikiLinkAutocomplete = class {
  ...
  getAllEntries() {
    ...
    for (const f of this.app.vault.getFiles()) {
      entries.push({ name: f.basename, folder: ((_a = f.parent) == null ? void 0 : _a.path) || "", isFile: true, mtime: f.stat.mtime });
  ...
  positionDropdown() {
    ...
    if (left + dropW > cr.width)
      left = Math.max(4, cr.width - dropW - 4);
  ...
  async renderPreview() {
    ...
    const tags = (_b = (_a = cache == null ? void 0 : cache.tags) == null ? void 0 : _a.map((t) => t.tag)) != null ? _b : [];
    const frontmatterTags = (_d = (_c = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _c.tags) != null ? _d : [];
    const allTags = [.../* @__PURE__ */ new Set([...tags, ...frontmatterTags])];
    const resolved = (_e = this.app.metadataCache.resolvedLinks) != null ? _e : {};
    let backlinkCount = 0;
    for (const src of Object.keys(resolved)) {
      if ((_f = resolved[src]) == null ? void 0 : _f[file.path])
        backlinkCount++;
    }
    ...
    if (allTags.length > 0) {
      ...
      for (const t of allTags)
        tagsEl.createSpan({ cls: "vin-preview-tag", text: String(t) });
    }
    this.previewEl.createDiv({ cls: "vin-preview-content", text: preview });
  }
  positionPreview() {
    if (!this.previewEl || !this.dropdownEl)
      return;
  ...
  destroy() {
    ...
    this.removePreview();
    this.removeDropdown();
    (_a = this.resizeDisposable) == null ? void 0 : _a.dispose();
  }
};
var BookmarkManager = class {
  ...
  addBookmark(label) {
    ...
    let decoration = null;
    try {
      decoration = (_a = this.terminal.registerDecoration({ marker, anchor: "left" })) != null ? _a : null;
      decoration == null ? void 0 : decoration.onRender((el) => {
  ...
  jumpTo(bookmark) {
    ...
    setTimeout(() => {
      var _a;
      return (_a = bookmark.pipEl) == null ? void 0 : _a.removeClass("is-active");
    }, 600);
  }
  jumpNext() {
    var _a;
    ...
    this.jumpTo((_a = sorted.find((b) => b.marker.line > vy + 1)) != null ? _a : sorted[0]);
  }
  jumpPrev() {
    var _a;
    if (!this.bookmarks.length)
    ...
    const sorted = [...this.bookmarks].sort((a, b) => a.marker.line - b.marker.line);
    const vy = this.terminal.buffer.active.viewportY;
    this.jumpTo((_a = sorted.slice().reverse().find((b) => b.marker.line < vy)) != null ? _a : sorted[sorted.length - 1]);
  }
  clearAll() {
  ...
  removeBookmark(bookmark) {
    ...
    (_a = bookmark.pipEl) == null ? void 0 : _a.remove();
    try {
      (_b = bookmark.decoration) == null ? void 0 : _b.dispose();
    } catch (e) {
    }
    try {
      (_c = bookmark.marker) == null ? void 0 : _c.dispose();
    } catch (e) {
    }
  }
  updateStrip() {
    ...
    for (const b of this.bookmarks) {
      if (b.pipEl)
        b.pipEl.style.top = `${b.marker.line / total * 100}%`;
    }
  }
  destroy() {
    if (this.updateTimer)
...
var TerminalSession = class {
  constructor(parent, id, cwd, app) {
    ...
    this.autocomplete = new WikiLinkAutocomplete(
      ...
      (data) => {
        var _a2;
        return (_a2 = this.process.stdin) == null ? void 0 : _a2.write(data);
      },
      this.containerEl
    );
    ...
    this.terminal.onData((data) => {
      var _a2, _b2;
      (_a2 = this.autocomplete) == null ? void 0 : _a2.handleData(data);
      (_b2 = this.process.stdin) == null ? void 0 : _b2.write(data);
    });
    (_a = this.process.stdout) == null ? void 0 : _a.on("data", (data) => {
      this.terminal.write(data);
      if (this._activityCallback)
        this._activityCallback(this);
    });
    ...
    if (this.textareaEl) {
      this.textareaEl.addEventListener("paste", (e) => {
        var _a2, _b2;
        if (!pluginSettings.stripFormattingOnPaste)
          return;
        const raw = (_b2 = (_a2 = e.clipboardData) == null ? void 0 : _a2.getData("text/plain")) != null ? _b2 : "";
        if (!raw)
          return;
        e.preventDefault();
        e.stopPropagation();
  ...
  showDropZone() {
    ...
    requestAnimationFrame(() => {
      var _a;
      return (_a = this.dropZoneEl) == null ? void 0 : _a.addClass("is-visible");
    });
  }
  hideDropZone() {
    if (!this.dropZoneEl)
  ...
  showDropBadge(filePaths) {
    var _a;
    if (this.dropBadgeTimer)
      clearTimeout(this.dropBadgeTimer);
    (_a = this.containerEl.querySelector(".vin-drop-badge")) == null ? void 0 : _a.remove();
  ...
  handleDrop(e, app) {
    ...
    if (paths.length === 0 && ((_d = (_c = e.dataTransfer) == null ? void 0 : _c.files) == null ? void 0 : _d.length)) {
      const images = Array.from(e.dataTransfer.files).filter((f) => f.type.startsWith("image/"));
      if (images.length) {
        this.saveDroppedImages(images);
        return;
      }
    }
    if (paths.length === 0 && e.dataTransfer) {
      const uriList = (_e = e.dataTransfer.getData("text/uri-list")) == null ? void 0 : _e.trim();
      if (uriList) {
        for (const uri of uriList.split("\n")) {
          ...
          if (trimmed.startsWith("file://")) {
            ...
            if (m)
              paths.push(pathMod.join(vaultPath, decodeURIComponent(m[1])));
          }
        }
      }
      if (!paths.length) {
        const plain = (_f = e.dataTransfer.getData("text/plain")) == null ? void 0 : _f.trim();
    ...
    if (!paths.length)
      return;
    (_g = this.process.stdin) == null ? void 0 : _g.write(paths.map((p) => this.shellEscape(p)).join(" "));
  ...
  async saveDroppedImages(files) {
    ...
    for (const file of files) {
      const ext = ((_a = file.type.split("/")[1]) == null ? void 0 : _a.replace("jpeg", "jpg")) || "png";
    ...
    if (!saved.length)
      return;
    (_b = this.process.stdin) == null ? void 0 : _b.write(saved.map((p) => this.shellEscape(p)).join(" "));
  ...
  captureOutput() {
    ...
    if (sel == null ? void 0 : sel.trim())
      return sel;
    const buf = this.terminal.buffer.active;
    const lines = [];
    for (let i = Math.max(0, buf.length - 50); i < buf.length; i++) {
      const line = (_a = buf.getLine(i)) == null ? void 0 : _a.translateToString(true);
      if (line !== void 0)
        lines.push(line);
    }
    return lines.join("\n").trimEnd();
  ...
  addBookmark(label) {
    var _a;
    (_a = this.bookmarkManager) == null ? void 0 : _a.addBookmark(label);
  }
  nextBookmark() {
    var _a;
    (_a = this.bookmarkManager) == null ? void 0 : _a.jumpNext();
  }
  prevBookmark() {
    var _a;
    (_a = this.bookmarkManager) == null ? void 0 : _a.jumpPrev();
  }
  clearBookmarks() {
    var _a;
    (_a = this.bookmarkManager) == null ? void 0 : _a.clearAll();
  }
  destroy() {
    var _a, _b;
    (_a = this.bookmarkManager) == null ? void 0 : _a.destroy();
    (_b = this.autocomplete) == null ? void 0 : _b.destroy();
    try {
...
var _FullscreenManager = class {
  ...
  enter(layout) {
    var _a;
    if (this.isOpen || _FullscreenManager.overlayOpen || !this.view.sessions.length)
      return;
    _FullscreenManager.overlayOpen = true;
    if (layout)
      this.layout = layout;
    this.focusedSession = (_a = this.view.activeSession) != null ? _a : this.view.sessions[0];
    ...
    this.overlay.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && !this.isRenaming) {
        const anyAc = this.view.sessions.some((s) => {
          var _a2;
          return (_a2 = s.autocomplete) == null ? void 0 : _a2.active;
        });
        if (!anyAc) {
          e.preventDefault();
          e.stopPropagation();
    ...
    requestAnimationFrame(() => {
      var _a2;
      return (_a2 = this.overlay) == null ? void 0 : _a2.classList.add("is-visible");
    });
  ...
  exit() {
    ...
    if (this.resizeTimer)
      clearTimeout(this.resizeTimer);
    const target = this.focusedSession && this.view.sessions.includes(this.focusedSession) ? this.focusedSession : this.view.sessions[0] || null;
    this.view.activeSession = null;
    if (target)
      this.view.switchTo(target);
    this.view.renderTabs();
    requestAnimationFrame(() => {
      var _a2, _b;
      (_a2 = this.view.activeSession) == null ? void 0 : _a2.fit();
      (_b = this.view.activeSession) == null ? void 0 : _b.focus();
    });
  }
  setLayout(layout) {
    if (layout === this.layout && this.gridEl)
      return;
    this.layout = layout;
    if (this.gridEl)
  ...
  saveAndMoveAll() {
    ...
    for (const s of this.view.sessions) {
      ...
      if (parent)
        this.savedPositions.set(s, { parent, nextSibling: s.containerEl.nextSibling });
    }
    this.renderFsTabs();
    this.rebuildPanes();
  }
  ...
  rebuildPanes() {
    var _a;
    if (!this.gridEl || this.isRenaming)
      return;
    this.gridEl.empty();
    ...
    for (const session of this.view.sessions) {
      if (!visibleSet.has(session)) {
        ...
        (_a = this.overlay) == null ? void 0 : _a.appendChild(session.containerEl);
      }
    }
    for (const session of visible) {
      const pane = this.gridEl.createDiv({ cls: "vin-fullscreen-pane" + (session === this.focusedSession ? " is-focused" : "") });
      if (multiPane)
        pane.createDiv({ cls: "vin-fullscreen-pane-label", text: session.name });
      ...
      pane.addEventListener("mousedown", () => {
        ...
        if (this.focusedSession !== session) {
          session.hasActivity = false;
          this.focusedSession = session;
          (_a2 = this.gridEl) == null ? void 0 : _a2.querySelectorAll(".vin-fullscreen-pane").forEach((p) => {
    ...
    for (const session of this.view.sessions) {
      session.setActivityCallback((s) => {
        ...
        if (s !== this.focusedSession && !s.hasActivity) {
          s.hasActivity = true;
          const tabs = (_a = this.tabBarEl) == null ? void 0 : _a.querySelectorAll(".vin-fs-tab");
          if (tabs) {
...
var TerminalView = class extends import_obsidian.ItemView {
  ...
  getState() {
    ...
    return {
      ...
      activeId: (_b = (_a = this.activeSession) == null ? void 0 : _a.id) != null ? _b : null,
      nextId: this.nextId
    };
  }
  async setState(state, result) {
    var _a, _b, _c, _d, _e;
    if (((_a = state == null ? void 0 : state.sessions) == null ? void 0 : _a.length) > 0) {
      for (const s of this.sessions)
        s.destroy();
      this.sessions = [];
      this.activeSession = null;
      this.nextId = (_b = state.nextId) != null ? _b : 1;
      const vaultPath = this.app.vault.adapter.basePath;
      for (const saved of state.sessions) {
        const id = (_c = saved.id) != null ? _c : this.nextId++;
        if (id >= this.nextId)
          this.nextId = id + 1;
        const session = new TerminalSession(this.sessionsEl, id, vaultPath, this.app);
        session.name = (_d = saved.name) != null ? _d : `zsh ${id}`;
        this.sessions.push(session);
        session.hide();
      }
      const target = (_e = this.sessions.find((s) => s.id === state.activeId)) != null ? _e : this.sessions[0];
      if (target)
  ...
  async onOpen() {
    ...
    this.resizeObserver = new ResizeObserver(() => {
      ...
      this.resizeTimer = setTimeout(() => {
        var _a;
        return (_a = this.activeSession) == null ? void 0 : _a.fit();
      }, 60);
    });
    ...
    let followDebounce = null;
    this.registerEvent(this.app.workspace.on("file-open", (file) => {
      var _a, _b;
      if (!pluginSettings.followActiveNote || !file || file.extension !== "md")
        return;
      if (followDebounce)
      ...
      followDebounce = setTimeout(() => {
        ...
        (_a2 = session.process.stdin) == null ? void 0 : _a2.write(`cd "${escaped}"
`);
      }, 300);
    }));
    this.createSession();
  ...
  resolveNoteCwd() {
    ...
    const vaultPath = this.app.vault.adapter.basePath;
    if (!vaultPath)
      return (_a = process.env.HOME) != null ? _a : "";
    try {
      const activeFile = this.app.workspace.getActiveFile();
      if (!activeFile || activeFile.extension !== "md")
        return vaultPath;
      const parentPath = (_c = (_b = activeFile.parent) == null ? void 0 : _b.path) != null ? _c : "";
  ...
  renderTabs() {
    ...
    fsBtn.addEventListener("click", () => {
      var _a;
      return (_a = this.fullscreenManager) == null ? void 0 : _a.toggle();
    });
  ...
  async onClose() {
    var _a, _b;
    (_a = this.fullscreenManager) == null ? void 0 : _a.destroy();
    this.fullscreenManager = null;
...
var TerminalSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Strip formatting on paste").setDesc("Remove ANSI color codes and escape sequences when pasting text into the terminal. Fixes garbled output when copying from Claude Code or other terminal emulators. Change takes effect on the next paste.").addToggle((t) => t.setValue(this.plugin.settings.stripFormattingOnPaste).onChange(async (v) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Auto-cd to active note on open").setDesc("When you open a new terminal tab, start in the directory of your currently active note instead of the vault root.").addToggle((t) => t.setValue(this.plugin.settings.autoCdOnOpen).onChange(async (v) => {
      this.plugin.settings.autoCdOnOpen = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Follow active note").setDesc("When you switch to a different note, automatically cd the terminal to that note's directory. 300ms debounced \u2014 rapid note switching sends only one cd command.").addToggle((t) => t.setValue(this.plugin.settings.followActiveNote).onChange(async (v) => {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (leaves.length)
          (_a = leaves[0].view.fullscreenManager) == null ? void 0 : _a.toggle();
      }
    });
    this.addCommand({
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE);
        if (leaves.length)
          (_a = leaves[0].view.activeSession) == null ? void 0 : _a.addBookmark();
      }
    });
    this.addCommand({
      id: "next-bookmark",
      ...
      callback: () => {
        ...
        const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE);
        if (leaves.length)
          (_a = leaves[0].view.activeSession) == null ? void 0 : _a.nextBookmark();
      }
    });
    this.addCommand({
      id: "prev-bookmark",
      ...
      callback: () => {
        ...
        const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE);
        if (leaves.length)
          (_a = leaves[0].view.activeSession) == null ? void 0 : _a.prevBookmark();
      }
    });
    this.addCommand({
      id: "clear-bookmarks",
      ...
      callback: () => {
        ...
        const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE);
        if (leaves.length)
          (_a = leaves[0].view.activeSession) == null ? void 0 : _a.clearBookmarks();
      }
    });
